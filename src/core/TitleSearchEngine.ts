...
export class TitleSearchEngine {
   ...
   */
  buildIndex(articles: ParsedArticle[]): Map<string, Array<{articleIndex: number, weight: number}>> {
    // 清空之前的映射
   ...
   */
  search(query: string, searchIndex: Map<string, Array<{articleIndex: number, weight: number}>>, searchableArticles: SearchableArticle[]): SearchResult[] {
    if (!query.trim()) {
      return [];
    }
