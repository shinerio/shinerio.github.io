## 跳过内容为空的Markdown文件
date: 2026-02-19
在文章解析阶段（src/index.ts）新增空内容过滤逻辑：解析每个Markdown文件后，若去除YAML frontmatter后的正文内容为空（仅有空白字符），则跳过该文件，不生成对应页面也不加入搜索索引，并在控制台输出提示信息。适用于Vault.md等无实质内容的索引型文件。
...
## Sidebar统计信息改为两行显示
date: 2026-02-17
将sidebar中的文章数量和更新时间从单行显示改为垂直两行显示，提升视觉层次感。修改SiteGenerator.ts的renderSidebar方法，将单行span元素拆分为两个独立的div.stat-item元素。更新CSS样式：.sidebar-stats使用flex-direction: column实现垂直排列，调整间距（gap: var(--spacing-xs)）和内边距（padding: var(--spacing-md)），新增.stat-item样式实现左对齐。

## 修复划词评论页面跳转问题
date: 2026-02-17
修复文章详情页划词评论点击后自动跳转到文章末尾的问题，保持scrollbar位置不变。问题根源是textarea.focus()调用时浏览器会自动滚动到该元素位置。修复方案：1) 调整代码执行顺序，先定位并显示popover再聚焦textarea；2) 使用textarea.focus({ preventScroll: true })防止自动滚动；3) 添加保存和恢复滚动位置的逻辑作为兼容性后备方案；4) 优化positionPopover函数确保在定位前能获取正确的popover尺寸。同时修复showCommentPopover和showEditPopover两个函数中的相同问题。
