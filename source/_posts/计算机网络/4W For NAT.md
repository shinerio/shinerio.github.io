...
> 以上的穿透NAT，都是对PNAT来进行穿透，主要是针对UDP协议，语音视频通信是用UDP传输的。Basic NAT不修改经过的数据包的端口号，它们可以看作是完全锥形NAT的精简版本，即Basic NAT也可以被穿透。NAT设备将在一定时间后关闭UDP的一个映射，所以为了保持与服务器能够一直通信，服务器或客户端必须要周期性地发送UDP包，保持映射不被关闭。
