...
其实我们不需要那么多数据，那么我们可以对db查询顺序进行优化，先对snat_rules表进行limit，这样关联的时候就只需要进行limit次扫描了。

```shell
explain analyze select snat_rules.id, snat_rules.nat_gateway_id, snat_rules.tenant_id, snat_rules.network_id, snat_rules.status, snat_rules.admin_state_up, snat_rules.tenant_id, snat_rules.created_at, snat_rules.source_type, snat_rules.description, snat_rules.cidr as cidr, snat_rule_fip_associations.floating_ip_id as floating_ip_id, snat_rule_fip_associations.floating_ip_address as floating_ip_address, snat_rule_fip_associations.admin_state as admin_state, snat_rule_fip_associations.br_vni, snat_rule_fip_associations.br_vtep, snat_rule_fip_associations.nat_vni, group_concat(distinct(ext_snat_rules.host_id)) as host_id, group_concat(distinct(ext_snat_rules.seq0)) as seq0, group_concat(distinct(ext_snat_rules.status)) as action_status, group_concat(distinct(ext_snat_rules.internal_vni)) as internal_vni from (select * from snat_rules where snat_rules.tenant_id = '057ef0819a80d2732fcdc0024a9a30a7' order by snat_rules.created_at asc limit '2000') snat_rules left join snat_rule_fip_associations on snat_rule_fip_associations.snat_rule_id=snat_rules.id left join ext_snat_rules on ext_snat_rules.id=snat_rules.id group by snat_rules.id, snat_rules.nat_gateway_id,snat_rules.tenant_id, snat_rules.network_id, snat_rules.status, snat_rules.admin_state_up, snat_rules.tenant_id, snat_rules.created_at, snat_rules.source_type, snat_rules.description, snat_rules.cidr,snat_rule_fip_associations.floating_ip_id ,snat_rule_fip_associations.floating_ip_address, snat_rule_fip_associations.admin_state,snat_rule_fip_associations.br_vni, snat_rule_fip_associations.br_vtep, snat_rule_fip_associations.nat_vni;

                                                                                  QUERY PLAN                                                                                   
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 GroupAggregate  (cost=58997.78..60982.81 rows=22686 width=290) (actual time=214.303..364.258 rows=23923 loops=1)
   ->  Sort  (cost=58997.78..59054.50 rows=22686 width=290) (actual time=214.261..216.992 rows=23923 loops=1)
         Sort Key: PUBLIC.SNAT_RULES.ID, PUBLIC.SNAT_RULES.NAT_GATEWAY_ID, PUBLIC.SNAT_RULES.TENANT_ID, PUBLIC.SNAT_RULES.NETWORK_ID, PUBLIC.SNAT_RULES.STATUS, PUBLIC.SNAT_RULES.ADMIN_STATE_UP, PUBLIC.SNAT_RULES.CREATED_AT, PUBLIC.SNAT_RULES.SOURCE_TYPE, PUBLIC.SNAT_RULES.DESCRIPTION, PUBLIC.SNAT_RULES.CIDR, SNAT_RULE_FIP_ASSOCIATIONS.FLOATING_IP_ID, SNAT_RULE_FIP_ASSOCIATIONS.FLOATING_IP_ADDRESS, SNAT_RULE_FIP_ASSOCIATIONS.ADMIN_STATE, SNAT_RULE_FIP_ASSOCIATIONS.BR_VNI, SNAT_RULE_FIP_ASSOCIATIONS.BR_VTEP, SNAT_RULE_FIP_ASSOCIATIONS.NAT_VNI
         Sort Method: quicksort  Memory: 13104kB
         ->  Nested Loop Left Join  (cost=2035.50..57356.50 rows=22686 width=290) (actual time=18.461..94.135 rows=23923 loops=1)
               ->  Nested Loop Left Join  (cost=2035.50..5951.28 rows=2000 width=215) (actual time=18.446..35.282 rows=2000 loops=1)
                     ->  Limit  (cost=2035.50..2040.50 rows=2000 width=277) (actual time=18.420..19.245 rows=2000 loops=1)
                           ->  Sort  (cost=2035.50..2072.21 rows=14685 width=277) (actual time=18.418..19.003 rows=2000 loops=1)
                                 Sort Key: PUBLIC.SNAT_RULES.CREATED_AT
                                 Sort Method: quicksort  Memory: 4285kB
                                 ->  Seq Scan on SNAT_RULES  (cost=0.00..1156.91 rows=14685 width=277) (actual time=0.016..10.225 rows=14684 loops=1)
                                       Filter: ((TENANT_ID)::TEXT = '057ef0819a80d2732fcdc0024a9a30a7'::TEXT)
                                       Rows Removed by Filter: 4348
                     ->  Index Scan using EXT_SNAT_RULES_PKEY on EXT_SNAT_RULES  (cost=0.00..1.94 rows=1 width=74) (actual time=0.007..0.007 rows=1 loops=2000)
                           Index Cond: ((ID)::TEXT = (PUBLIC.SNAT_RULES.ID)::TEXT)
               ->  Index Scan using SNAT_RULE_FIP_ASSOCIATIONS_PKEY on SNAT_RULE_FIP_ASSOCIATIONS  (cost=0.00..25.59 rows=11 width=112) (actual time=0.009..0.027 rows=12 loops=2000)
                     Index Cond: ((SNAT_RULE_ID)::TEXT = (PUBLIC.SNAT_RULES.ID)::TEXT)
 Total runtime: 365.603 ms
(18 rows)
```

> 可以看出，时间直接降低到了365.603ms，基本可以满足我们的正常使用，对SNAT_RULE_FIP_ASSOCIATIONS的扫描只花费0.027 * 2000 = 54ms，相对于原来的190 + 220 = 410ms提升巨大。
...
explain analyze select snat_rules.id, snat_rules.nat_gateway_id, snat_rules.tenant_id, snat_rules.network_id, snat_rules.status, snat_rules.admin_state_up, snat_rules.tenant_id, snat_rules.created_at, snat_rules.source_type, snat_rules.description, snat_rules.cidr as cidr, snat_rule_fip_associations.floating_ip_id as floating_ip_id, snat_rule_fip_associations.floating_ip_address as floating_ip_address, snat_rule_fip_associations.admin_state as admin_state, snat_rule_fip_associations.br_vni, snat_rule_fip_associations.br_vtep, snat_rule_fip_associations.nat_vni, group_concat(distinct(ext_snat_rules.host_id)) as host_id, group_concat(distinct(ext_snat_rules.seq0)) as seq0, group_concat(distinct(ext_snat_rules.status)) as action_status, group_concat(distinct(ext_snat_rules.internal_vni)) as internal_vni from (select * from snat_rules where snat_rules.tenant_id = '057ef0819a80d2732fcdc0024a9a30a7' order by snat_rules.created_at asc limit '1') snat_rules left join snat_rule_fip_associations on snat_rule_fip_associations.snat_rule_id=snat_rules.id left join ext_snat_rules on ext_snat_rules.id=snat_rules.id group by snat_rules.id, snat_rules.nat_gateway_id,snat_rules.tenant_id, snat_rules.network_id, snat_rules.status, snat_rules.admin_state_up, snat_rules.tenant_id, snat_rules.created_at, snat_rules.source_type, snat_rules.description, snat_rules.cidr,snat_rule_fip_associations.floating_ip_id ,snat_rule_fip_associations.floating_ip_address, snat_rule_fip_associations.admin_state,snat_rule_fip_associations.br_vni, snat_rule_fip_associations.br_vtep, snat_rule_fip_associations.nat_vni;
                                                                                ...
                                                                                QUERY PLAN                                                                                 
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 GroupAggregate  (cost=1259.48..1260.44 rows=11 width=290) (actual time=21.539..21.539 rows=1 loops=1)
   ->  Sort  (cost=1259.48..1259.51 rows=11 width=290) (actual time=21.512..21.512 rows=1 loops=1)
         Sort Key: PUBLIC.SNAT_RULES.ID, PUBLIC.SNAT_RULES.NAT_GATEWAY_ID, PUBLIC.SNAT_RULES.TENANT_ID, PUBLIC.SNAT_RULES.NETWORK_ID, PUBLIC.SNAT_RULES.STATUS, PUBLIC.SNAT_RULES.ADMIN_STATE_UP, PUBLIC.SNAT_RULES.CREATED_AT, PUBLIC.SNAT_RULES.SOURCE_TYPE, PUBLIC.SNAT_RULES.DESCRIPTION, PUBLIC.SNAT_RULES.CIDR, SNAT_RULE_FIP_ASSOCIATIONS.FLOATING_IP_ID, SNAT_RULE_FIP_ASSOCIATIONS.FLOATING_IP_ADDRESS, SNAT_RULE_FIP_ASSOCIATIONS.ADMIN_STATE, SNAT_RULE_FIP_ASSOCIATIONS.BR_VNI, SNAT_RULE_FIP_ASSOCIATIONS.BR_VTEP, SNAT_RULE_FIP_ASSOCIATIONS.NAT_VNI
         Sort Method: quicksort  Memory: 25kB
         ->  Nested Loop Left Join  (cost=1230.34..1259.29 rows=11 width=290) (actual time=21.475..21.479 rows=1 loops=1)
               ->  Nested Loop Left Join  (cost=1230.34..1232.65 rows=1 width=215) (actual time=21.457..21.458 rows=1 loops=1)
                     ->  Limit  (cost=1230.34..1230.34 rows=1 width=277) (actual time=21.434..21.435 rows=1 loops=1)
                           ->  Sort  (cost=1230.34..1267.05 rows=14685 width=277) (actual time=21.432..21.432 rows=1 loops=1)
                                 Sort Key: PUBLIC.SNAT_RULES.CREATED_AT
                                 Sort Method: quicksort  Memory: 4285kB
                                 ->  Seq Scan on SNAT_RULES  (cost=0.00..1156.91 rows=14685 width=277) (actual time=0.012..11.427 rows=14684 loops=1)
                                       Filter: ((TENANT_ID)::TEXT = '057ef0819a80d2732fcdc0024a9a30a7'::TEXT)
                                       Rows Removed by Filter: 4348
                     ->  Index Scan using EXT_SNAT_RULES_PKEY on EXT_SNAT_RULES  (cost=0.00..2.29 rows=1 width=74) (actual time=0.016..0.016 rows=1 loops=1)
                           Index Cond: ((ID)::TEXT = (PUBLIC.SNAT_RULES.ID)::TEXT)
               ->  Index Scan using SNAT_RULE_FIP_ASSOCIATIONS_PKEY on SNAT_RULE_FIP_ASSOCIATIONS  (cost=0.00..26.52 rows=11 width=112) (actual time=0.013..0.016 rows=1 loops=1)
                     Index Cond: ((SNAT_RULE_ID)::TEXT = (PUBLIC.SNAT_RULES.ID)::TEXT)
 Total runtime: 21.682 ms
(18 rows)
```

> 可以看到优化前，即使limit为1也需要接近1217ms，而优化的sql只需要21.682就能完成。

### 用户list接口

上述测试的是最复杂的情况，而实际用户使用的时并不会关联非ext表，此情况下对比如下。

```shell
explain analyze select snat_rules.id, snat_rules.nat_gateway_id, snat_rules.tenant_id, snat_rules.network_id, snat_rules.status, snat_rules.admin_state_up, snat_rules.tenant_id, snat_rules.created_at, snat_rules.cidr as cidr, snat_rules.source_type, snat_rules.description, group_concat(distinct(snat_rule_fip_associations.floating_ip_id)) as floating_ip_id, group_concat(distinct(snat_rule_fip_associations.floating_ip_address)) as floating_ip_address, group_concat(distinct(snat_fip_freezed.floating_ip_address)) as freezedIpAddress from snat_rules left join snat_rule_fip_associations on snat_rule_fip_associations.snat_rule_id=snat_rules.id left join ext_snat_rules ext_snat_rules on ext_snat_rules.id=snat_rules.id left join snat_rule_fip_associations snat_fip_freezed on snat_fip_freezed.snat_rule_id=snat_rules.id and snat_fip_freezed.admin_state = 'FREEZED' where snat_rules.tenant_id = '057ef0819a80d2732fcdc0024a9a30a7' group by snat_rules.id order by snat_rules.created_at asc limit '2000';

                                                                                                   QUERY PLAN                                                                                                    
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=634219.00..634224.00 rows=2000 width=241) (actual time=760.686..761.364 rows=2000 loops=1)
   ->  Sort  (cost=634219.00..634255.71 rows=14685 width=241) (actual time=760.684..761.117 rows=2000 loops=1)
         Sort Key: SNAT_RULES.CREATED_AT
         Sort Method: quicksort  Memory: 15292kB
         ->  GroupAggregate  (cost=0.00..633340.41 rows=14685 width=241) (actual time=0.204..735.510 rows=14684 loops=1)
               ->  Nested Loop Left Join  (cost=0.00..631668.54 rows=141489 width=241) (actual time=0.083..472.941 rows=176076 loops=1)
                     ->  Nested Loop Left Join  (cost=0.00..316393.61 rows=14685 width=191) (actual time=0.075..281.937 rows=14684 loops=1)
                           ->  Index Scan using SNAT_RULES_PKEY on SNAT_RULES  (cost=0.00..2183.34 rows=14685 width=178) (actual time=0.016..29.115 rows=14684 loops=1)
                                 Filter: ((TENANT_ID)::TEXT = '057ef0819a80d2732fcdc0024a9a30a7'::TEXT)
                                 Rows Removed by Filter: 4349
                           ->  Index Scan using SNAT_RULE_FIP_ASSOCIATIONS_PKEY on SNAT_RULE_FIP_ASSOCIATIONS SNAT_FIP_FREEZED  (cost=0.00..21.39 rows=1 width=50) (actual time=0.017..0.017 rows=0 loops=14684)
                                 Index Cond: ((SNAT_RULE_ID)::TEXT = (SNAT_RULES.ID)::TEXT)
                                 Filter: ((ADMIN_STATE)::TEXT = 'FREEZED'::TEXT)
                                 Rows Removed by Filter: 12
                     ->  Index Scan using SNAT_RULE_FIP_ASSOCIATIONS_PKEY on SNAT_RULE_FIP_ASSOCIATIONS  (cost=0.00..21.36 rows=11 width=87) (actual time=0.004..0.010 rows=12 loops=14684)
                           Index Cond: ((SNAT_RULE_ID)::TEXT = (SNAT_RULES.ID)::TEXT)
 Total runtime: 763.194 ms
(17 rows)

explain analyze select snat_rules.id, snat_rules.nat_gateway_id, snat_rules.tenant_id, snat_rules.network_id, snat_rules.status, snat_rules.admin_state_up, snat_rules.created_at, snat_rules.source_type, snat_rules.description, snat_rules.cidr as cidr, snat_rule_fip_associations.floating_ip_id as floating_ip_id, snat_rule_fip_associations.floating_ip_address as floating_ip_address, snat_rule_fip_associations.admin_state as admin_state from (select * from snat_rules where 1 = 1 and snat_rules.tenant_id = '057ef0819a80d2732fcdc0024a9a30a7' order by snat_rules.id asc limit '2000' ) snat_rules left join snat_rule_fip_associations on snat_rule_fip_associations.snat_rule_id=snat_rules.id left join ext_snat_rules ext_snat_rules on ext_snat_rules.id=snat_rules.id where 1 = 1 group by snat_rules.id, snat_rules.nat_gateway_id, snat_rules.tenant_id, snat_rules.network_id, snat_rules.status, snat_rules.admin_state_up, snat_rules.created_at, snat_rules.source_type, snat_rules.description, snat_rules.cidr, snat_rule_fip_associations.floating_ip_id, snat_rule_fip_associations.floating_ip_address, snat_rule_fip_associations.admin_state;

                                                                                  QUERY PLAN                                                                                   
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 HashAggregate  (cost=52459.88..52686.74 rows=22686 width=235) (actual time=101.229..121.867 rows=23989 loops=1)
   ->  Nested Loop Left Join  (cost=0.00..51722.58 rows=22686 width=235) (actual time=0.047..58.093 rows=23989 loops=1)
         ->  Limit  (cost=0.00..297.36 rows=2000 width=277) (actual time=0.021..5.903 rows=2000 loops=1)
               ->  Index Scan using SNAT_RULES_PKEY on SNAT_RULES  (cost=0.00..2183.34 rows=14685 width=277) (actual time=0.020..5.622 rows=2000 loops=1)
                     Filter: ((TENANT_ID)::TEXT = '057ef0819a80d2732fcdc0024a9a30a7'::TEXT)
                     Rows Removed by Filter: 570
         ->  Index Scan using SNAT_RULE_FIP_ASSOCIATIONS_PKEY on SNAT_RULE_FIP_ASSOCIATIONS  (cost=0.00..25.59 rows=11 width=94) (actual time=0.007..0.023 rows=12 loops=2000)
               Index Cond: ((SNAT_RULE_ID)::TEXT = (PUBLIC.SNAT_RULES.ID)::TEXT)
 Total runtime: 123.818 ms
(9 rows)
```
...
```shell
select count(*) as num, host_id from ext_snat_rules where status!='delete' group by host_id;

  NUM  |        HOST_ID        
-------+-----------------------
   217 | nat_gateway_cluster01
  4107 | nat_gateway_cluster02
    13 | nat_gateway_cluster05
     1 | nat_gateway_cluster07
 14695 | nat_gateway_cluster06
```

选取规则最多的两个集群测试如下(优化后对于非NORMAL状态的数据放在了业务代码中执行)：

```shell
explain analyze select snat_rules.id, snat_rules.nat_gateway_id, snat_rules.tenant_id, snat_rules.network_id, snat_rules.status, snat_rules.admin_state_up, snat_rules.tenant_id, snat_rules.created_at, group_concat(distinct(ext_snat_rules.cidr)) as cidr, snat_rules.source_type, snat_rules.description, (array_agg(snat_rule_fip_associations.br_vni))[1] as br_vni, (array_agg(snat_rule_fip_associations.br_vtep))[1] as br_vtep, (array_agg(snat_rule_fip_associations.nat_vni))[1] as nat_vni, group_concat(distinct(ext_snat_rules.host_id)) as host_id, group_concat(distinct(ext_snat_rules.seq0)) as seq0, group_concat(distinct(ext_snat_rules.status)) as action_status, group_concat(distinct(ext_snat_rules.internal_vni)) as internal_vni, group_concat(distinct(snat_rule_fip_associations.floating_ip_id)) as floating_ip_id, group_concat(distinct(snat_rule_fip_associations.floating_ip_address)) as floating_ip_address from snat_rules left join snat_rule_fip_associations on snat_rule_fip_associations.snat_rule_id=snat_rules.id and snat_rule_fip_associations.admin_state = 'NORMAL' left join ext_snat_rules ext_snat_rules on ext_snat_rules.id=snat_rules.id left join snat_rule_fip_associations snat_fip_freezed on snat_fip_freezed.snat_rule_id=snat_rules.id and snat_fip_freezed.admin_state = 'FREEZED' where ext_snat_rules.host_id in ('nat_gateway_cluster06', 'nat_gateway_cluster02') group by snat_rules.id order by snat_rules.id asc ,snat_rules.created_at asc limit '2000';

                                                                                                   QUERY PLAN                                                                                                    
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 Limit  (cost=720901.07..720906.07 rows=2000 width=282) (actual time=1401.397..1401.736 rows=2000 loops=1)
   ->  Sort  (cost=720901.07..720948.65 rows=19033 width=282) (actual time=1401.396..1401.585 rows=2000 loops=1)
         Sort Key: SNAT_RULES.ID, SNAT_RULES.CREATED_AT
         Sort Method: quicksort  Memory: 17799kB
         ->  GroupAggregate  (cost=0.00..719762.35 rows=19033 width=282) (actual time=0.185..1371.847 rows=18802 loops=1)
               ->  Nested Loop Left Join  (cost=0.00..712160.87 rows=163184 width=282) (actual time=0.068..653.724 rows=180195 loops=1)
                     ->  Nested Loop Left Join  (cost=0.00..365450.48 rows=16938 width=214) (actual time=0.063..381.763 rows=18802 loops=1)
                           ->  Nested Loop  (cost=0.00..20433.89 rows=16938 width=214) (actual time=0.027..129.508 rows=18802 loops=1)
                                 ->  Index Scan using SNAT_RULES_PKEY on SNAT_RULES  (cost=0.00..2135.75 rows=19033 width=163) (actual time=0.010..24.541 rows=19033 loops=1)
                                 ->  Index Scan using EXT_SNAT_RULES_PKEY on EXT_SNAT_RULES  (cost=0.00..0.95 rows=1 width=88) (actual time=0.005..0.005 rows=1 loops=19033)
                                       Index Cond: ((ID)::TEXT = (SNAT_RULES.ID)::TEXT)
                                       Filter: ((HOST_ID)::TEXT = ANY ('{nat_gateway_cluster06,nat_gateway_cluster02}'::TEXT[]))
                                       Rows Removed by Filter: 0
                           ->  Index Scan using SNAT_RULE_FIP_ASSOCIATIONS_PKEY on SNAT_RULE_FIP_ASSOCIATIONS SNAT_FIP_FREEZED  (cost=0.00..20.36 rows=1 width=37) (actual time=0.013..0.013 rows=0 loops=18802)
                                 Index Cond: ((SNAT_RULE_ID)::TEXT = (SNAT_RULES.ID)::TEXT)
                                 Filter: ((ADMIN_STATE)::TEXT = 'FREEZED'::TEXT)
                                 Rows Removed by Filter: 10
                     ->  Index Scan using SNAT_RULE_FIP_ASSOCIATIONS_PKEY on SNAT_RULE_FIP_ASSOCIATIONS  (cost=0.00..20.36 rows=11 width=105) (actual time=0.004..0.012 rows=10 loops=18802)
                           Index Cond: ((SNAT_RULE_ID)::TEXT = (SNAT_RULES.ID)::TEXT)
                           Filter: ((ADMIN_STATE)::TEXT = 'NORMAL'::TEXT)
 Total runtime: 1403.048 ms
(21 rows)

explain analyze select snat_rules.id, snat_rules.nat_gateway_id, snat_rules.tenant_id, snat_rules.network_id, snat_rules.status, snat_rules.admin_state_up, snat_rules.created_at, snat_rules.source_type, snat_rules.description, group_concat(distinct(ext_snat_rules.cidr)) as cidr, group_concat(distinct(ext_snat_rules.host_id)) as host_id, group_concat(distinct(ext_snat_rules.seq0)) as seq0, group_concat(distinct(ext_snat_rules.status)) as action_status, group_concat(distinct(ext_snat_rules.internal_vni)) as internal_vni, snat_rule_fip_associations.br_vni as br_vni, snat_rule_fip_associations.br_vtep as br_vtep, snat_rule_fip_associations.nat_vni as nat_vni, snat_rule_fip_associations.floating_ip_id as floating_ip_id, snat_rule_fip_associations.floating_ip_address as floating_ip_address, snat_rule_fip_associations.admin_state as admin_state from (select * from snat_rules where 1 = 1 order by snat_rules.id asc ,snat_rules.created_at asc limit '2000' ) snat_rules left join snat_rule_fip_associations on snat_rule_fip_associations.snat_rule_id=snat_rules.id left join ext_snat_rules ext_snat_rules on ext_snat_rules.id=snat_rules.id where 1 = 1 and ext_snat_rules.host_id in ('nat_gateway_cluster06', 'nat_gateway_cluster02') group by snat_rules.id, snat_rules.nat_gateway_id, snat_rules.tenant_id, snat_rules.network_id, snat_rules.status, snat_rules.admin_state_up, snat_rules.created_at, snat_rules.source_type, snat_rules.description, snat_rule_fip_associations.br_vni, snat_rule_fip_associations.br_vtep, snat_rule_fip_associations.nat_vni, snat_rule_fip_associations.floating_ip_id, snat_rule_fip_associations.floating_ip_address, snat_rule_fip_associations.admin_state;

                                                                                                   QUERY PLAN                                                                                                    
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 GroupAggregate  (cost=53368.13..55185.14 rows=20189 width=289) (actual time=121.260..286.171 rows=19272 loops=1)
   ->  Sort  (cost=53368.13..53418.60 rows=20189 width=289) (actual time=121.215..123.286 rows=19272 loops=1)
         Sort Key: PUBLIC.SNAT_RULES.ID, PUBLIC.SNAT_RULES.NAT_GATEWAY_ID, PUBLIC.SNAT_RULES.TENANT_ID, PUBLIC.SNAT_RULES.NETWORK_ID, PUBLIC.SNAT_RULES.STATUS, PUBLIC.SNAT_RULES.ADMIN_STATE_UP, PUBLIC.SNAT_RULES.CREATED_AT, PUBLIC.SNAT_RULES.SOURCE_TYPE, PUBLIC.SNAT_RULES.DESCRIPTION, SNAT_RULE_FIP_ASSOCIATIONS.BR_VNI, SNAT_RULE_FIP_ASSOCIATIONS.BR_VTEP, SNAT_RULE_FIP_ASSOCIATIONS.NAT_VNI, SNAT_RULE_FIP_ASSOCIATIONS.FLOATING_IP_ID, SNAT_RULE_FIP_ASSOCIATIONS.FLOATING_IP_ADDRESS, SNAT_RULE_FIP_ASSOCIATIONS.ADMIN_STATE
         Sort Method: quicksort  Memory: 10706kB
         ->  Nested Loop Left Join  (cost=2248.05..51924.48 rows=20189 width=289) (actual time=37.786..83.280 rows=19272 loops=1)
               ->  Nested Loop  (cost=2248.05..6173.83 rows=1780 width=214) (actual time=37.762..48.917 rows=1991 loops=1)
                     ->  Limit  (cost=2248.05..2253.05 rows=2000 width=277) (actual time=37.726..38.339 rows=2000 loops=1)
                           ->  Sort  (cost=2248.05..2295.64 rows=19033 width=277) (actual time=37.726..38.075 rows=2000 loops=1)
                                 Sort Key: PUBLIC.SNAT_RULES.ID, PUBLIC.SNAT_RULES.CREATED_AT
                                 Sort Method: quicksort  Memory: 5824kB
                                 ->  Seq Scan on SNAT_RULES  (cost=0.00..1109.33 rows=19033 width=277) (actual time=0.013..13.425 rows=19033 loops=1)
                     ->  Index Scan using EXT_SNAT_RULES_PKEY on EXT_SNAT_RULES  (cost=0.00..1.94 rows=1 width=88) (actual time=0.004..0.004 rows=1 loops=2000)
                           Index Cond: ((ID)::TEXT = (PUBLIC.SNAT_RULES.ID)::TEXT)
                           Filter: ((HOST_ID)::TEXT = ANY ('{nat_gateway_cluster06,nat_gateway_cluster02}'::TEXT[]))
                           Rows Removed by Filter: 0
               ->  Index Scan using SNAT_RULE_FIP_ASSOCIATIONS_PKEY on SNAT_RULE_FIP_ASSOCIATIONS  (cost=0.00..25.59 rows=11 width=112) (actual time=0.005..0.015 rows=10 loops=1991)
                     Index Cond: ((SNAT_RULE_ID)::TEXT = (PUBLIC.SNAT_RULES.ID)::TEXT)
 Total runtime: 288.191 ms
(18 rows)
```

> 优化前1403ms，优化后288ms。filter的时间基本可以忽略，从优化前的执行分析也可以看到，filter的耗时是很短的。