(function () {
  ...
  function buildShell(state) {
    ...
    if (state.sourceToc) {
      state.tocHotspot = document.createElement('div');
      state.tocHotspot.className = 'article-presenter-toc-hotspot';
      state.presenterToc = buildToc(state);
      state.stage.appendChild(state.tocHotspot);
